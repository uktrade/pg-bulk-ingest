...
- If an intermediate table is used due to a migration or `delete=Delete.BEFORE_FIRST_BATCH`:

  - `SELECT` permissions are copied from the old table to the new table.

  - Views (including materialized views) are dropped and recreated on the new table.
